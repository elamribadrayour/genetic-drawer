...
impl Fitness for Mae {
    fn calculate(&self, source: &RgbaImage, result: &RgbaImage) -> f32 {
        ...
        for (pixel, target_pixel) in source.pixels().zip(result.pixels()) {
            error += (pixel[0] as f32 - target_pixel[0] as f32).abs()
                + (pixel[1] as f32 - target_pixel[1] as f32).abs()
                + (pixel[2] as f32 - target_pixel[2] as f32).abs()
                + (pixel[3] as f32 - target_pixel[3] as f32).abs();
        }
